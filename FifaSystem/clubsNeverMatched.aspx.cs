        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session["user"] != null)
            {
                ...
                else
                {
                    ...
                    trial.CommandText = "Select * From clubsNeverMatched";

                    conn.Open();
                    SqlDataReader rdr = trial.ExecuteReader(CommandBehavior.CloseConnection);
                    while (rdr.Read())
                    {
                        String hostName = rdr.GetString(rdr.GetOrdinal("First Club"));
                        String guestName = rdr.GetString(rdr.GetOrdinal("Second Club"));
